...
import { RegistryGraph } from "@/app/components/RegistryGraph";
...
type RegistryPathNode = { label: string; fullName: string; registry: string; resolver?: string; hasResolver: boolean; isLeaf: boolean };
type Result = {
  input: string;
...
// These now come straight from the same constants the backend calls (src/ens/raw.ts),
// instead of being duplicated by hand here — that duplication is exactly what let the
// UI and the actual resolution logic silently drift apart before.
const V2_TARGET = UNIVERSAL_RESOLVER_V2_SEPOLIA;
const CANONICAL = UNIVERSAL_RESOLVER;
...
export default function Home() {
  ...
  async function inspect() {
    ...
    try {
      ...
      const response = await fetch(`/api/inspect?name=${encodeURIComponent(name.trim())}&_=${cacheBust}`, {
        cache: "no-store",
  ...
  const ccipSteps = trace.filter(s => s.id.startsWith("ccip-read-"));
  ...
  return <main className="shell">
    ...
    {result && <div className="results">
      ...
      {result.usedCcipRead && <div className="fallback-note ccip"><b>⇄ CCIP-READ (EIP-3668)</b><span>This resolver answered via an offchain gateway lookup instead of onchain storage. See the CCIP-Read section below for the exact gateway request and response.</span></div>}
      ...
      <Section eyebrow="01 / hierarchy" title="Registry trace" count={registrySteps.length}>
        {result.registryPath && result.registryPath.length > 0 && (
          <RegistryGraph path={result.registryPath} rootRegistry={result.registryPath[0]?.registry} />
        )}
        <div className="trace-panel">{registrySteps.map((s,i) => <TraceRow key={s.id} step={s} index={i}/>)}</div>
      </Section>
      ...
      {ccipSteps.length > 0 && (
        <Section eyebrow="03b / offchain" title="CCIP-Read gateway calls" count={ccipSteps.length}>
          <div className="trace-panel">{ccipSteps.map((s,i) => <TraceRow key={s.id} step={s} index={i}/>)}</div>
        </Section>
      )}
